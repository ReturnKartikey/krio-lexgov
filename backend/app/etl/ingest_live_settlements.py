...
async def ingest_live_sebi_settlement_orders():
    ...
    listing_url = "https://www.sebi.gov.in/sebiweb/home/HomeAction.do?doListing=yes&sid=2&ssid=9&smid=3"
    ...
    async with httpx.AsyncClient(timeout=20.0, follow_redirects=True) as client:
        ...
        async with AsyncSessionLocal() as session:
            src = (await session.execute(select(Source).where(Source.adapter_key == "sebi_adjudication_orders"))).scalar_one()
            ...
            for row in rows:
                ...
                existing = (await session.execute(select(Record).where(Record.source_url == order_url))).scalar_one_or_none()
                if existing:
                    continue
                ...
                if iframe and iframe.has_attr('src'):
                    ...
                    try:
                        ...
                        if pdf_resp.status_code == 200:
                            ...
                            amt_matches = re.findall(r'(?:Rs\.?|INR|₹)\s*([\d,]+(?:\.\d+)?)', pdf_text)
                            candidates = []
                ...
                entity_names = [e.name for e in extracted_entities] if extracted_entities else ["Noticee Entity"]
                ...
                record = Record(
                    ...
                    title=title,
                    summary=f"Settlement Order passed by SEBI under Settlement Proceedings Regulations, 2018 in the matter of {title}. Settlement amount of Rs. {settlement_amount:,.2f} remitted by the applicant." if settlement_amount else f"Settlement Order passed by SEBI in the matter of {title}.",
                ...
                session.add(record)
                added += 1
                print(f"Added [{added}] {title[:45]}... | Settlement: Rs. {settlement_amount} | State: {state_val}")
